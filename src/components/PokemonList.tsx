...
export function PokemonList({ initialData, initialTypes }: PokemonListProps) {
  // Seed the state with server-fetched data so the first paint has content
  const [pokemon, setPokemon] = useState<PokemonResult[]>(initialData.results);
  ...
  // Main data fetching logic — only runs when user interacts (search, filter, paginate)
  // The very first render uses server-fetched initialData, so this skips the initial load.
  useEffect(() => {
    // Skip fetching on initial mount when there's no user interaction
    if (offset === 0 && !selectedType && !searchQuery) return;
